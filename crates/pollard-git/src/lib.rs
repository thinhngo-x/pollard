...
fn write_dir(repo: &gix::Repository, tree: ObjectId, dir: &Path) -> Result<()> {
    ...
    let entries: Vec<_> = tree
        ...
        .map(|e| {
            (
                dir.join(OsStr::from_bytes(e.filename)),
                e.mode.kind(),
                e.oid.to_owned(),
            )
        })
    ...
    for (path, kind, oid) in entries {
        match kind {
            ...
            EntryKind::Link => {
                ...
                std::os::unix::fs::symlink(OsStr::from_bytes(&blob.data), &path)
                    .map_err(io(&path))?;
            }
...
pub fn export(
    ...
    for (m, message) in commits {
        let tree = write_tree(store, m, &mut |kind, data| {
            repo.objects.write_buf(kind, data).map_err(git_err)
        })?;
    ...
    let Some(tip) = parent.filter(|_| !commits.is_empty()) else {
        return Ok(ids);
    };
    repo.reference(
...
mod tests {
    ...
    fn matches_git_write_tree() {
        ...
        let m = store
            .snapshot_dir(&wc, &WalkOptions::default())
            .unwrap()
            .manifest;
        git(&wc, &["init", "-q"]);
        git(&wc, &["add", "-A"]);
        ...
        assert_eq!(
            tree_hash(&store, &Manifest::default()).unwrap(),
    ...
    fn import_export_roundtrip() {
        ...
        let store = Store::new(t.path().join("dot"));
        let m = store
            .snapshot_dir(&out, &pollard_objects::WalkOptions::default())
            .unwrap()
            .manifest;
        ...
        m2.entries.push(Entry {
            path: ".pollard-recipe.json".into(),
            size,
            hash: h,
            mode: 0o100644,
        });
        ...
        assert_eq!(
            git(&wc, &["rev-list", "--parents", "-n1", "exp"]),
            format!("{} {}", ids[1], ids[0])
        );
        assert_eq!(
            git(&wc, &["rev-list", "--parents", "-n1", &ids[0]]),
            format!("{} {head}", ids[0])
        );
        assert_eq!(
            git(&wc, &["rev-parse", &format!("{}^{{tree}}", ids[0])]),
            head_tree
        );
