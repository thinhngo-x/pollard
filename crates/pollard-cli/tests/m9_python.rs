...
fn sdk_and_plain_file_io_are_equivalent() {
    ...
    for step in [10.0, 20.0, 30.0] {
        assert!(
            has_num(&la, step) && has_num(&la, 1.0 / step),
            "SDK run missing step {step}:\n{la}"
        );
        assert!(
            has_num(&lb, step) && has_num(&lb, 1.0 / step),
            "plain run missing step {step}:\n{lb}"
        );
    }
    ...
    assert!(
        sa.contains("src.pt"),
        "SDK save_checkpoint not registered:\n{sa}"
    );
    assert!(
        sb.contains("w.pt"),
        "plain-I/O checkpoint not registered:\n{sb}"
    );
}
...
fn current_raises_outside_pollard_run() {
    ...
    let o = r.exec(c);
    assert!(
        o.stdout.contains("RAISED"),
        "pollard.current() did not raise outside `pollard run`:\n{o}"
    );
}
...
fn siblings_helper_returns_table() {
    ...
    let mut cmd = r.cmd_in(
        &r.root,
        Path::new("python3"),
        &[
            "-c",
            &format!("import pollard\nt = pollard.siblings({p:?})\nprint(t)\n"),
        ],
    );
    cmd.env("PYTHONPATH", sdk_path());
    let o = r.exec(cmd);
    assert!(
        o.ok() && o.stdout.contains(&c),
        "pollard.siblings() failed or lacks child {c}:\n{o}"
    );
}
...
fn sdk_core_is_pure_python_under_200_lines() {
    ...
    for e in std::fs::read_dir(&dir)
        .expect("pollard package dir")
        .flatten()
    {
        let p = e.path();
        let ext = p.extension().and_then(|x| x.to_str()).unwrap_or("");
        assert!(
            !matches!(ext, "so" | "pyd" | "c" | "pyx"),
            "compiled extension in SDK: {}",
            p.display()
        );
    ...
    assert!(
        core_lines < 200,
        "SDK core is {core_lines} lines (target < 200, §4)"
    );
}
...
fn uv_tool_install_from_built_wheel() {
    ...
    let o = r.exec(r.cmd_in(
        &python_dir(),
        Path::new("uv"),
        &["build", "--wheel", "-o", dist.to_str().unwrap()],
    ));
    ...
    let wheel = std::fs::read_dir(&dist)
        ...
        .expect("no wheel built");
    let (tool_dir, bin_dir) = (tmp.path().join("tools"), tmp.path().join("bin"));
    let mut c = r.cmd_in(
        &r.root,
        Path::new("uv"),
        &["tool", "install", wheel.to_str().unwrap()],
    );
    c.env("UV_TOOL_DIR", &tool_dir)
    ...
    assert!(
        installed.exists(),
        "`pollard` not on the tool bin dir after install:\n{o}"
    );
    let o = r.exec(r.cmd_in(&r.root, &installed, &["init"]));
    assert!(
        o.ok() && r.root.join(".pollard").is_dir(),
        "installed pollard binary does not work:\n{o}"
    );
}
