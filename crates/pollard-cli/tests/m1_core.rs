...
fn init_creates_repo_layout_and_gitignore_entry() {
    ...
    assert!(r.root.join(".pollard/db.sqlite").is_file(), ".pollard/db.sqlite missing (§3)");
    let gi = r.read(".gitignore");
    assert!(gi.contains("target/"), "init clobbered existing .gitignore:\n{gi}");
    assert!(gi.lines().any(|l| l.trim().trim_matches('/') == ".pollard"), ".pollard not added to .gitignore:\n{gi}");
}
...
fn three_runs_on_200_file_tree_under_1s_each() {
    ...
    for i in 0..3 {
        ...
        assert!(o.elapsed < budget(Duration::from_secs(1)), "run {i} took {:?} (spec: < 1 s)\n{o}", o.elapsed);
    }
}
...
fn tree_shows_parent_links_and_note_titles() {
    ...
    for (id, title) in [(&a, "alpha title"), (&b, "beta title"), (&c, "gamma title")] {
        ...
        assert!(line.contains(title), "note title {title:?} not on {id}'s tree line:\n{t}");
    }
    assert!(!t.contains("alpha body paragraph"), "tree must show only the first line of the note:\n{t}");
    ...
    assert_eq!(col_of(&t, &b), col_of(&t, &c), "siblings b,c at different depths:\n{t}");
    // show carries the parent explicitly.
    let sb = r.ok(&["show", &b]).stdout;
    assert!(sb.contains(&a), "show {b} does not mention parent {a}:\n{sb}");
}
...
fn run_without_m_gets_auto_note_from_config_delta() {
    ...
    assert!(s.to_lowercase().contains("auto"), "auto note not flagged `auto` in show:\n{s}");
    assert!(s.contains("lr"), "auto note does not mention the changed key `lr`:\n{s}");
    let t = r.ok(&["tree"]).stdout;
...
fn m_dash_reads_note_from_stdin() {
    ...
    let mut child = c.stdout(std::process::Stdio::piped()).stderr(std::process::Stdio::piped()).spawn().unwrap();
    use std::io::Write;
    child.stdin.take().unwrap().write_all(b"from stdin title\nbody\n").unwrap();
    let o = child.wait_with_output().unwrap();
    assert!(o.status.success(), "{}", String::from_utf8_lossy(&o.stderr));
    let id = String::from_utf8_lossy(&o.stdout).lines().last().unwrap().trim().to_string();
    let t = r.ok(&["tree"]).stdout;
    assert!(line_with(&t, &id).unwrap().contains("from stdin title"), "{t}");
}
...
fn undo_after_run_removes_node_and_restores_working_copy() {
    ...
    assert_eq!(r.current(), a, "current node not restored to parent after undo");
    let after = r.snapshot();
    let names = |v: &Vec<(String, Vec<u8>)>| v.iter().map(|x| x.0.clone()).collect::<Vec<_>>();
    assert_eq!(names(&after), names(&before), "working-copy file set differs after undo\n{o}");
...
fn duplicate_recipe_refused_without_force() {
    ...
    assert!(o.all().contains(&a), "refusal must name the existing node {a} (§8 errors name the node):\n{o}");
    let t = r.ok(&["tree"]).stdout;
    assert_eq!(ids_in(&t).len(), 1, "refused run still created a node:\n{t}");
...
fn run_status_done_and_failed() {
    ...
    let id = o.last_line();
    assert!(is_node_id(&id), "failed run must still print its node id last:\n{o}");
    let s = r.ok(&["show", &id]).stdout;
...
fn show_prints_full_record() {
    ...
    let hexes: Vec<usize> = s.split(|c: char| !c.is_ascii_hexdigit()).map(str::len).collect();
    assert!(hexes.iter().any(|&n| n == 40), "no 40-hex git tree hash in show (§7 code hash):\n{s}");
    assert!(hexes.iter().any(|&n| n == 64 || (n >= 8 && n < 40)), "no blake3 hash (full or abbreviated) in show:\n{s}");
}
...
fn note_sets_and_replaces_note_last_line_is_id_and_undo_restores() {
    ...
    r.ok(&["undo"]);
    let t = r.ok(&["tree"]).stdout;
    assert!(line_with(&t, &a).unwrap().contains("original"), "undo did not restore note:\n{t}");
}
...
fn node_references_full_prefix_at_and_at_minus() {
    ...
    assert_eq!(first_id(&r.ok(&["show", "@-"]).stdout).unwrap(), a, "`@-` is not the parent");
    // shortest prefix of b that a does not share
    let p = (1..=b.len()).map(|n| &b[..n]).find(|p| !a.starts_with(p)).unwrap();
    assert_eq!(first_id(&r.ok(&["show", p]).stdout).unwrap(), b, "unique prefix {p:?} did not resolve");
...
fn run_records_exact_command() {
    ...
    let a = r.run(&["-m", "a"], "true");
    let s = r.ok(&["show", &a]).stdout;
    assert!(s.contains("sh ") && s.contains("run"), "show does not include the launch command:\n{s}");
}
...
fn duplicate_of_failed_node_is_allowed_with_notice() {
    ...
    assert!(o.ok(), "relaunching a failed recipe must not be refused:\n{o}");
    assert!(o.all().contains(&failed), "no notice naming the failed duplicate {failed}:\n{o}");
...
fn fork_restores_code_and_config_and_prints_id() {
    ...
    assert!(!r.root.join("attn.py").exists(), "attn.py (not in {a}) left behind");
...
fn fork_then_undo_restores_uncommitted_edits_byte_identically() {
    ...
    let after = r.snapshot();
    let names = |v: &Vec<(String, Vec<u8>)>| v.iter().map(|x| x.0.clone()).collect::<Vec<_>>();
    assert_eq!(names(&after), names(&before), "file set differs after fork+undo");
    assert!(after == before, "uncommitted edits not restored byte-identically after fork+undo");
    assert_eq!(r.current(), b, "undo of fork did not restore the current node");
}
...
fn code_hash_equals_git_write_tree() {
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(r.root.join("run.sh"), std::fs::Permissions::from_mode(0o755)).unwrap();
    }
    let a = r.run(&["-m", "a"], "true");
    let tree = r.sh("git init -q . && git add -A && git write-tree").stdout.trim().to_string();
    let s = r.ok(&["show", &a]).stdout;
    assert!(s.contains(&tree), "code hash in `show {a}` != `git write-tree` {tree}:\n{s}");
}
