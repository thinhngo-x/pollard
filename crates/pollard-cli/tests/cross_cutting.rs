...
fn import_last_line_and_undo() {
    let r = Repo::init();
    r.sh("git init -q -b main . && git add -A && git -c user.name=t -c user.email=t@e commit -qm i");
...
fn undo_of_undo_is_not_required_but_undo_on_empty_log_fails_cleanly() {
    let r = Repo::init();
    let o = r.po(&["undo"]);
    assert!(!o.ok() || o.all().to_lowercase().contains("nothing"), "undo with empty op log should fail or say nothing to undo:\n{o}");
}
...
fn commands_under_200ms_on_1000_node_repo() {
    ...
    for i in 1..1000 {
        ...
        ids.push(r.run(&["-m", &format!("n{i}"), "--parent", &parent], &format!("emit 1 loss={i}")));
    }
    ...
    for c in &checks {
        let o = r.ok(c);
        if o.elapsed > budget { slow.push(format!("{c:?}: {:?}", o.elapsed)); }
    }
    assert!(slow.is_empty(), "commands over 200 ms on a 1000-node repo:\n{}", slow.join("\n"));
}
...
fn siblings_100_children_fast() {
    ...
    for i in 0..100 {
        r.write("config.yaml", format!("lr: {i}\n"));
        r.run(&["-m", &format!("c{i}"), "--parent", &p], &format!("emit 1 loss={i}"));
    }
    let o = r.ok(&["siblings", &p]);
    // Spec says 50 ms for the join; 200 ms here covers process start + rendering (generous-but-real).
    assert!(o.elapsed < budget(Duration::from_millis(200)), "siblings on 100 children took {:?}", o.elapsed);
}
...
fn ctrl_c_during_run_marks_node_killed() {
    let r = Repo::init();
    let s = r.script("k.sh", "emit 1 loss=1; emit 2 loss=2; sleep 0.3; kill -INT 0; sleep 5; emit 3 loss=3");
    let mut c = r.cmd_in(&r.root, &bin(), &["run", "-m", "k", "--", "sh", &s]);
    std::os::unix::process::CommandExt::process_group(&mut c, 0);
    let o = r.exec(c);
    let id = o.last_line();
    assert!(is_node_id(&id), "interrupted run must still print its node id last:\n{o}");
    let show = r.ok(&["show", &id]).stdout;
    assert!(show.contains("killed"), "Ctrl-C'd run should be `killed`:\n{show}");
    let log = r.ok(&["log", &id, "--key", "loss"]).stdout;
    assert!(has_num(&log, 2.0) && !log.lines().any(|l| l.trim_start().starts_with('3')), "metrics before the kill not ingested (or after):\n{log}");
}
