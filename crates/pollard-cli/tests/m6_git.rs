...
fn tree_without_recipe(r: &Repo, rev: &str) -> String {
    ...
    std::fs::read_to_string(format!("{}.out", idx.display()))
        .unwrap()
        .trim()
        .to_string()
}
...
fn init_from_git_imports_head_as_root_with_git_tree_hash() {
    ...
    assert!(
        is_node_id(&root),
        "init --from-git must print the root id last:\n{o}"
    );
    assert!(
        o.stdout.contains(&head_tree[..7]) || r.ok(&["show", &root]).stdout.contains(&head_tree),
        "root's code hash is not HEAD's tree hash {head_tree}"
    );
    let s = r.ok(&["show", &root]).stdout;
    assert!(
        s.contains(&head_tree),
        "show root lacks code = {head_tree}:\n{s}"
    );
}
...
fn import_rev_creates_root_node() {
    ...
    assert!(
        s.contains(&first_tree),
        "import HEAD~1: code != its tree {first_tree}:\n{s}"
    );
}
...
fn import_then_export_tree_equals_head_and_head_untouched() {
    ...
    assert!(
        s.contains(&head_tree),
        "imported node's code != HEAD's tree {head_tree}:\n{s}"
    );
    let o = r.ok(&["export", &id, "--branch", "exp1"]);
    assert_eq!(
        tree_without_recipe(&r, "exp1"),
        head_tree,
        "exported tree minus .pollard-recipe.json != HEAD's tree\n{o}"
    );
    assert_eq!(git(&r, "rev-parse HEAD"), head, "HEAD moved");
    assert_eq!(
        git(&r, "symbolic-ref HEAD"),
        "refs/heads/main",
    ...
    assert_eq!(
        ...
        status_before,
        "index/working tree touched by export"
    );
}
...
fn export_single_run_node_has_generated_message() {
    ...
    assert!(
        first.contains(&id) && first.contains("lower lr"),
        "first line must carry node id and note title:\n{msg}"
    );
    assert!(
        msg.contains("lr"),
        "body should carry the config delta:\n{msg}"
    );
    let files = git(&r, "ls-tree -r --name-only one");
    assert!(
        files.contains("REPORT.md"),
        "off-tree file missing from export:\n{files}"
    );
...
fn export_path_four_node_chain() {
    ...
    r.ok(&[
        "export",
        "--path",
        &format!("{}..{}", chain[0], chain[3]),
        "--branch",
        "chain",
    ]);
    let revs: Vec<String> = git(&r, "rev-list --reverse chain ^main")
        .lines()
        .map(String::from)
        .collect();
    let revs = if revs.len() == 4 {
        revs
    } else {
        git(&r, "rev-list --reverse chain")
            .lines()
            .map(String::from)
            .collect()
    };
    assert_eq!(
        revs.len(),
        4,
        "expected 4 commits on the exported branch, got {}",
        revs.len()
    );
    assert_eq!(
        git(&r, "rev-list --min-parents=2 --count chain"),
        "0",
        "export history is not linear"
    );
    for (i, (rev, id)) in revs.iter().zip(&chain).enumerate() {
        let subject = git(&r, &format!("log -1 --format=%s {rev}"));
        assert!(
            subject.contains(id.as_str()),
            "commit {i} subject lacks node id {id}: {subject}"
        );
        let files = git(&r, &format!("ls-tree -r --name-only {rev}"));
        assert!(
            files.contains(".pollard-recipe.json"),
            "commit {i} lacks .pollard-recipe.json:\n{files}"
        );
        assert!(
            files.contains("REPORT.md"),
            "commit {i} lacks the off-tree REPORT.md:\n{files}"
        );
        assert_eq!(
            git(&r, &format!("show {rev}:REPORT.md")),
            "# report CURRENT",
            "v3 §7: off-tree files come from the current working copy"
        );
        let recipe = git(&r, &format!("show {rev}:.pollard-recipe.json"));
        assert!(
            recipe.trim_start().starts_with('{'),
            "recipe file not JSON:\n{recipe}"
        );
        if i > 0 {
            assert_eq!(
                git(&r, &format!("show {rev}:src/model.py")),
                format!("x = {i}"),
                "commit {i} has wrong code"
            );
        }
    }
    assert_eq!(git(&r, "rev-parse HEAD"), head, "HEAD moved");
...
fn export_path_accepts_pin_names() {
    ...
    r.ok(&[
        "export",
        "--path",
        &format!("{root}..paper-v1"),
        "--branch",
        "paper-v1",
    ]);
    let n = [
        git(&r, "rev-list --count paper-v1 ^main"),
        git(&r, "rev-list --count paper-v1"),
    ];
    assert!(
        n.iter().any(|c| c == "2"),
        "expected a 2-commit export for root..paper-v1, got {n:?}"
    );
}
...
fn pollard_never_touches_git_index_outside_export() {
    ...
    r.ok(&["tree"]);
    assert!(
        std::fs::read(r.root.join(".git/index")).unwrap() == idx,
        "git index changed by run/tree"
    );
}
