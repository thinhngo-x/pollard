...
fn run_sets_protocol_env_vars() {
    ...
    assert!(
        env.contains("fork=UNSET"),
        "POLLARD_FORK_STEP must be unset on a fresh run:\n{env}"
    );
}
...
fn run_prints_node_id_before_script_starts() {
    ...
    let started = o
        .stdout
        .find("SCRIPT-STARTED")
        .expect("script stdout not passed through");
    assert!(
        first_id_pos.is_some_and(|p| p < started),
        "node id not printed before the script's output:\n{o}"
    );
}
...
emit 3 loss=0.125"#,
    ...
    assert_eq!(
        ...
        "LIVE",
        "metrics were not visible via `log` during the run (10 s budget)"
    );
    let pts = points(&r.ok(&["log", &id, "--key", "loss"]).stdout);
    assert_eq!(
        pts,
        vec![(1, 0.5), (2, 0.25), (3, 0.125)],
        "remainder not ingested on exit"
    );
}
...
fn metrics_ingested_on_exit_multiple_keys_per_line() {
    ...
    let id = r.run(
        &["-m", "a"],
        "emit 10 loss=2.5 acc=0.1; emit 20 loss=2.0 acc=0.2",
    );
...
fn fork_step_inheritance_and_monotonicity() {
    ...
    let a = r.run(
        &["-m", "A"],
        "s=1; while [ $s -le 150 ]; do emit $s loss=$s; s=$((s+1)); done",
    );
    ...
    assert_eq!(
        std::fs::read_to_string(&envf).unwrap().trim(),
        "100",
        "POLLARD_FORK_STEP not set to 100 in B"
    );
    ...
    assert_eq!(
        pts.len(),
        200,
        "log B should return 200 points, got {}",
        pts.len()
    );
    ...
    assert_eq!(
        steps,
        (1..=200).collect::<Vec<_>>(),
        "steps not the continuous 1..=200"
    );
    assert!(pts.contains(&(100, 100.0)), "step 100 should be A's value");
    assert!(
        pts.contains(&(150, 1150.0)),
        "step 150 should be B's own value, not A's"
    );
    ...
    assert!(
        o.all().contains(&a),
        "no one-line notice naming the re-parent target {a}:\n{o}"
    );
    ...
    assert!(
        sc.contains(&a) && !sc.contains(&b),
        "C should be re-parented to A, not B:\n{sc}"
    );
...
fn node_may_log_key_parent_never_logged() {
    ...
    assert_eq!(
        points(&r.ok(&["log", &b, "--key", "acc"]).stdout),
        vec![(3, 0.9)]
    );
...
fn plain_child_without_fork_step_inherits_nothing() {
    ...
    assert_eq!(
        points(&r.ok(&["log", &b, "--key", "loss"]).stdout),
        vec![(5, 5.0)]
    );
}
...
fn siblings_last_common_and_last_own_cells() {
    ...
    let p = r.run(
        &["-m", "P"],
        "s=10; while [ $s -le 100 ]; do emit $s loss=5; s=$((s+10)); done",
    );
    r.write("config.yaml", "lr: 1\ndepth: 12\nact: relu\n");
    // C1 stops at 50 with loss 4 → last_common = 50 (C3 failed, excluded).
    let c1 = r.run(
        &["-m", "c1", "--parent", &p],
        "s=10; while [ $s -le 50 ]; do emit $s loss=4; s=$((s+10)); done",
    );
    r.write("config.yaml", "lr: 2\ndepth: 12\nact: relu\n");
    // C2 runs to 80: loss 3 through step 50, 2.5 at its final step.
    let c2 = r.run(
        &["-m", "c2", "--parent", &p],
        "s=10; while [ $s -le 70 ]; do emit $s loss=3; s=$((s+10)); done; emit 80 loss=2.5",
    );
    ...
    assert!(
        metric_lines.len() >= 2,
        "expected a last_common row and a last_own row for `loss`:\n{out}"
    );
    // last_common row: label mentions step 50; C1 = 4 (−1), C2 = 3 (−2).
    let common = metric_lines
        .iter()
        .find(|l| has_num(l, 3.0) && has_num(l, -2.0) && has_num(l, 4.0) && has_num(l, -1.0))
        .unwrap_or_else(|| panic!("no last_common row with C1=4(−1) and C2=3(−2):\n{out}"));
    assert!(
        common.contains("50"),
        "last_common row should be labelled with step 50:\n{out}"
    );
    // last_own row: C1 = 4 at step 50, C2 = 2.5 at step 80 (−2.5 vs parent's 5 at 80).
    assert!(
        metric_lines
            .iter()
            .any(|l| has_num(l, 2.5) && has_num(l, -2.5) && has_num(l, 4.0)),
        "no last_own row with C1=4 and C2=2.5(−2.5):\n{out}"
    );
    assert!(
        out.contains("failed"),
        "status row should show C3 failed:\n{out}"
    );
    let _ = (c1, c2);
}
...
fn metric_key_defaults_to_first_alphabetically() {
    ...
    assert!(
        out.contains("alpha") && !out.contains("zeta"),
        "default metric should be `alpha`:\n{out}"
    );
}
...
fn pollard_config_sdk_capture() {
    ...
    let a = r.run(
        &["-m", "a"],
        "echo '{\"lr\": 0.1, \"opt\": {\"name\": \"adam\"}}' > \"$POLLARD_CONFIG\"; emit 1 loss=1",
    );
    let b = r.run(
        &["-m", "b", "--force"],
        "echo '{\"lr\": 0.2, \"opt\": {\"name\": \"adam\"}}' > \"$POLLARD_CONFIG\"; emit 1 loss=1",
    );
    let d = r.ok(&["diff", &a, &b]).stdout;
    assert!(
        d.contains("lr") && d.contains("0.1") && d.contains("0.2"),
        "config written via POLLARD_CONFIG not captured:\n{d}"
    );
...
fn log_accepts_node_refs() {
    ...
    r.run(&["-m", "a"], "emit 1 loss=1");
    assert_eq!(
        points(&r.ok(&["log", "@", "--key", "loss"]).stdout),
        vec![(1, 1.0)]
    );
}
...
fn ckpt_dir_defaults_to_ckpt_and_is_not_code() {
    ...
    r.run(
        &["-m", "a"],
        &format!(
            "echo \"$POLLARD_CKPT_DIR\" > {}; echo w > \"$POLLARD_CKPT_DIR/step1.pt\"",
            res.display()
        ),
    );
    ...
    assert_eq!(
        d.canonicalize().ok(),
        r.root.join("ckpt").canonicalize().ok(),
        "POLLARD_CKPT_DIR is not <repo>/ckpt"
    );
    r.write("ckpt/step2.pt", "more weights");
    let o = r.run_out(&["-m", "b"], "true");
    assert!(
        !o.ok(),
        "files in ckpt/ changed the recipe (must be auto-ignored):\n{o}"
    );
}
