...
fn two_clones_push_children_of_same_parent_no_collisions() {
    ...
    for f in ["nodes.jsonl", "objects"] {
        assert!(
            remote.join(f).exists(),
            "remote lacks {f} after push (§5 layout)"
        );
    }
    ...
    r2.ok(&["pull"]);
    assert!(
        r2.ok(&["tree"]).stdout.contains(&p),
        "clone 2 does not see {p} after pull"
    );
    ...
    assert_ne!(
        c1, c2,
        "id collision between clones (ids must be salted per clone)"
    );
    r1.ok(&["push"]);
    r2.ok(&["push"]);
    r1.ok(&["pull"]);
    ...
    for (name, r) in [("clone1", &r1), ("clone2", &r2)] {
        ...
        for id in [&p, &c1, &c2] {
            assert!(
                ...
                "{name} missing {id} after pull:\n{t}"
            );
        }
        assert!(
            r.ok(&["show", &c1]).stdout.contains(&p) && r.ok(&["show", &c2]).stdout.contains(&p),
            "{name}: parent links lost in sync"
        );
    }
    ...
    assert_eq!(
        listing(&remote),
        before,
        "second push changed the remote:\n{o1}\n{o2}"
    );
}
...
fn push_pull_transfers_checkpoints_and_second_push_is_noop() {
    ...
    let a = r1.run(
        &["-m", "A"],
        &format!(
            "emit 10 loss=1; cp {} \"$POLLARD_CKPT_DIR/step10.pt\"",
            r1.aux.join("c.pt").display()
        ),
    );
    r1.ok(&["push"]);
    ...
    assert!(
        bytes >= 4 * MB as u64,
        "push uploaded only {bytes} bytes; the 5 MB checkpoint's chunks are missing"
    );
    r1.ok(&["push"]);
    assert_eq!(
        listing(&remote),
        after_first,
        "second push transferred data"
    );

    let r2 = clone_with_remote(&remote);
    r2.ok(&["pull"]);
    r2.ok(&["fork", &a, "--step", "10", "--no-sync"]);
    let restored = std::fs::read(r2.root.join("ckpt/step10.pt"))
        .expect("ckpt/step10.pt not restored in clone 2");
    assert!(
        restored == data,
        "checkpoint pulled from remote is not byte-identical"
    );
}
...
fn push_uploads_only_missing_chunks() {
    ...
    r.run(
        ...
        &format!(
            "cp {} \"$POLLARD_CKPT_DIR/w.pt\"",
            r.aux.join("b.pt").display()
        ),
    );
    r.ok(&["push"]);
    let s2: u64 = listing(&remote).iter().map(|x| x.1).sum();
    assert!(
        s2 - s1 < (a.len() / 5) as u64,
        "second push uploaded {} bytes for a 1%-different 8 MB checkpoint",
        s2 - s1
    );
}
...
fn notes_and_pins_sync_last_writer_wins() {
    ...
    r2.ok(&["pull"]);
    assert_eq!(
        first_id(&r2.ok(&["show", "best"]).stdout).as_deref(),
        Some(a.as_str()),
        "pin not pulled"
    );
    ...
    r1.ok(&["pull"]);
    assert!(
        r1.ok(&["show", &a]).stdout.contains("renamed in clone 2"),
        "note update did not sync (last writer wins)"
    );
}
