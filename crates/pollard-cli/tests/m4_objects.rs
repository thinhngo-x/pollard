...
fn find(r: &Repo, name: &str) -> Option<PathBuf> {
    fn go(d: &std::path::Path, name: &str) -> Option<PathBuf> {
        for e in std::fs::read_dir(d).ok()? {
            let p = e.ok()?.path();
            if p.file_name().is_some_and(|f| f == ".pollard") { continue; }
            if p.is_dir() { if let Some(x) = go(&p, name) { return Some(x); } } else if p.file_name().is_some_and(|f| f == name) { return Some(p); }
        }
        None
    }
    go(&r.root, name)
...
fn run_ckpt(r: &Repo, note: &str, src: &std::path::Path, name: &str, step: u32) -> String {
    r.run(&["-m", note], &format!("emit {step} loss=1; cp {} \"$POLLARD_CKPT_DIR/{name}\"", src.display()))
}
...
fn dir_bytes(d: &std::path::Path) -> u64 {
    ...
    rd.flatten().map(|e| { let p = e.path(); if p.is_dir() { dir_bytes(&p) } else { p.metadata().unwrap().len() } }).sum()
}
...
fn checkpoints_differing_by_1pct_share_95pct_of_chunks() {
    ...
    assert!(n_a >= 300, "only {n_a} chunk files for a 50 MB checkpoint; expected ~800 (target 64 KB, max 128 KB)");
    ...
    assert!(shared >= 0.95, "B added {new_chunks} new chunks on top of A's {n_a}: shared by count {:.1}% < 95%", shared * 100.0);
    assert!(shared_b >= 0.95, "B added {new_bytes} chunk bytes on top of A's {bytes_a}: shared by bytes {:.1}% < 95%", shared_b * 100.0);
}
...
fn fork_step_restores_byte_identical_checkpoint() {
    ...
    while let Some(p) = find(&r, "step100.pt") { std::fs::remove_file(p).unwrap(); }
    let o = r.ok(&["fork", &a, "--step", "100", "--no-sync"]);
    assert_eq!(o.last_line(), a, "{o}");
    let p = find(&r, "step100.pt").unwrap_or_else(|| panic!("step100.pt not restored anywhere in the working copy:\n{o}"));
    assert!(std::fs::read(&p).unwrap() == data, "restored {} is not byte-identical", p.display());
}
...
fn fork_step_env_points_resumed_run_at_restored_checkpoint() {
    ...
    r.run(&["-m", "B"], &format!("cmp \"$POLLARD_CKPT_DIR/step100.pt\" {} && echo SAME > {}", src.display(), res.display()));
    assert_eq!(std::fs::read_to_string(res).unwrap_or_default().trim(), "SAME", "resumed run does not find the restored checkpoint in $POLLARD_CKPT_DIR");
}
...
fn ckpt_command_registers_immediately_from_inside_script() {
    ...
    assert!(during.contains("step5"), "checkpoint not visible in `show` right after `pollard ckpt` (during run):\n{during}");
...
fn artifact_command_stores_output_under_weights_manifest() {
    ...
    assert!(r.chunk_count() > 0, "a 3 MB artifact (>1 MB) must be chunked");
}
...
fn large_file_never_stored_whole() {
    ...
    fn go(d: &std::path::Path, m: &mut u64) { if let Ok(rd) = std::fs::read_dir(d) { for e in rd.flatten() { let p = e.path(); if p.is_dir() { go(&p, m) } else { *m = (*m).max(p.metadata().unwrap().len()) } } } }
    go(&r.root.join(".pollard/chunks"), &mut biggest);
    go(&r.root.join(".pollard/objects"), &mut biggest);
    assert!(biggest <= 1024 * 1024, "a {biggest}-byte blob in the store: files > 1 MB must be chunked (max chunk 128 KB)");
}
...
fn prune_then_gc_frees_only_pruned_unique_chunks() {
    ...
    assert_eq!(r.chunk_count(), n_ab, "prune alone must not delete chunks (gc does)");
    r.ok(&["gc"]);
    assert_eq!(r.chunk_count(), n_a, "after prune+gc chunk count should return to A's {n_a}");
    // A's checkpoint is intact; B's recipe and metrics stay.
    while let Some(p) = find(&r, "step100.pt") { std::fs::remove_file(p).unwrap(); }
    r.ok(&["fork", &a, "--step", "100", "--no-sync"]);
    assert!(std::fs::read(find(&r, "step100.pt").unwrap()).unwrap() == a_bytes, "gc damaged A's checkpoint");
    let sb = r.ok(&["show", &b]).stdout;
    assert!(sb.contains("pruned"), "{sb}");
    assert!(r.ok(&["log", &b, "--key", "loss"]).stdout.contains("100"), "pruned node's metrics must remain queryable");
}
...
fn prune_marks_whole_subtree_and_undo_restores() {
    ...
    r.ok(&["prune", &b]);
    assert!(r.ok(&["show", &c]).stdout.contains("pruned"), "descendant C not pruned with B");
    assert!(!r.ok(&["show", &a]).stdout.contains("pruned"));
    let t = r.ok(&["tree"]).stdout;
    assert!(!t.contains(&c), "pruned subtree not collapsed in default tree:\n{t}");
    let ta = r.ok(&["tree", "--all"]).stdout;
    assert!(ta.contains(&b) && ta.contains(&c), "tree --all hides pruned nodes:\n{ta}");
    r.ok(&["undo"]);
    assert!(!r.ok(&["show", &b]).stdout.contains("pruned"), "undo did not un-prune B");
    assert!(!r.ok(&["show", &c]).stdout.contains("pruned"), "undo did not un-prune C");
}
...
fn siblings_exclude_pruned_unless_all() {
    ...
    assert!(!s.contains(&c1) && s.contains(&c2), "pruned child shown without --all:\n{s}");
...
fn cdc_insertion_stability_property_test_exists() {
    ...
    fn go(d: &std::path::Path, s: &mut String) { if let Ok(rd) = std::fs::read_dir(d) { for e in rd.flatten() { let p = e.path(); if p.is_dir() { go(&p, s) } else if p.extension().is_some_and(|x| x == "rs") { s.push_str(&std::fs::read_to_string(p).unwrap()) } } } }
    go(&dir.join("src"), &mut src);
    go(&dir.join("tests"), &mut src);
    assert!(src.contains("proptest!"), "no proptest! block in pollard-objects");
    assert!(src.to_lowercase().contains("insert"), "no insertion-stability property in pollard-objects");
}
...
fn fork_step_uses_largest_checkpoint_at_or_below_n_with_notice() {
    ...
    assert!(o.all().contains("20"), "no notice that step 20 was used for --step 25:\n{o}");
    assert_eq!(std::fs::read_to_string(r.root.join("ckpt/step20.pt")).unwrap_or_default(), "w20\n", "step20.pt not restored:\n{o}");
    r.write("config.yaml", "lr: 1\n");
    let res = r.aux.join("fs.txt");
    r.run(&["-m", "B"], &format!("echo $POLLARD_FORK_STEP > {}", res.display()));
    assert_eq!(std::fs::read_to_string(res).unwrap().trim(), "20", "fork_step should be 20, not 25");
}
...
fn fork_step_finds_checkpoint_in_ancestor() {
    let r = Repo::init();
    let a = r.run(&["-m", "A"], "emit 10 loss=1; echo w10 > \"$POLLARD_CKPT_DIR/step10.pt\"; emit 20 loss=1");
    r.ok(&["fork", &a, "--step", "10", "--no-sync"]);
    r.write("config.yaml", "lr: 1\n");
    let b = r.run(&["-m", "B"], "emit 30 loss=1");
    std::fs::remove_dir_all(r.root.join("ckpt")).ok();
    let o = r.ok(&["fork", &b, "--step", "10", "--no-sync"]);
    assert_eq!(std::fs::read_to_string(r.root.join("ckpt/step10.pt")).unwrap_or_default(), "w10\n", "ancestor A's step10.pt not found via B:\n{o}");
}
...
fn undo_prune_after_gc_warns_weights_gone() {
    ...
    r.ok(&["gc"]);
    let o = r.ok(&["undo"]);
    assert!(!r.ok(&["show", &a]).stdout.contains("pruned"), "undo did not restore the node");
    assert!(o.stderr.to_lowercase().contains("weight") || o.stderr.to_lowercase().contains("gc"), "no warning that weights are gone:\n{o}");
}
