...
pub fn parse_table(text: &str, ids: &[String]) -> (Vec<String>, BTreeMap<String, Vec<String>>) {
    let norm = |c: &str| {
        let c = c.trim().replace("->", "→").replace('−', "-").replace(['"', '\''], "").replace(" → ", "→").replace("→ ", "→").replace(" →", "→");
        if matches!(c.as_str(), "—" | "–" | "-" | "") { String::new() } else { c }
    };
    ...
    let split = |l: &str| -> Vec<String> {
        if bordered {
            ...
            for ch in l.trim_end().chars() {
                ...
                if spaces >= 2 && !cur.is_empty() { cells.push(norm(&cur)); cur.clear(); } else if spaces == 1 { cur.push(' '); }
                spaces = 0;
                cur.push(ch);
            }
            cells.push(norm(&cur));
            if l.starts_with("  ") { cells.insert(0, String::new()); }
            cells
        }
    };
    let mut header = vec![];
    ...
    for l in text.lines() {
        if l.trim().is_empty() || l.chars().all(|c| "─━┼┬┴├┤┌┐└┘+-=╞╪╡ │|".contains(c)) { continue; }
        let cells = split(l);
        if header.is_empty() {
...
fn five_children() -> (Repo, String, Vec<String>) {
    ...
    let mut child = |cfg: &str, add_attn: bool, rm_util: bool, note: &str| {
        ...
        if add_attn { r.write("attn.py", "class Attn: pass\n") } else if r.root.join("attn.py").exists() { r.rm("attn.py") }
        if rm_util { if r.root.join("util.py").exists() { r.rm("util.py") } } else { r.write("util.py", "def u(): pass\n") }
        kids.push(r.run(&["-m", note, "--parent", &p], "true"));
    };
    child("lr: 3\ndepth: 24\nact: relu\n", false, false, "c1 deeper");
    child("lr: 3\ndepth: 12\nact: gelu\n", false, false, "c2 gelu");
    child(base_cfg, true, false, "c3 attn");
    child("lr: 3\ndepth: 24\nact: relu\n", true, false, "c4 deeper+attn");
...
fn sibling_table_matches_hand_written_expected_table() {
    ...
    assert_eq!(header, kids, "columns must be one per child ordered by created_at:\n{out}");
    ...
    expected.insert("code".to_string(), e(["", "", "+attn.py", "+attn.py", "-util.py"]));
    ...
    let pos = |k: &str| out.lines().position(|l| l.trim_start_matches(['│', '|', ' ']).starts_with(k)).unwrap();
    assert!(pos("depth") < pos("code") && pos("act") < pos("code") && pos("code") < pos("status"), "row grouping order wrong:\n{out}");
}
...
fn siblings_default_is_current_nodes_parent() {
    ...
    assert_eq!(a, b, "`siblings` with no arg should equal `siblings <parent of @>`");
}
...
"#,
    ...
    let o = r.exec(r.cmd_in(&r.aux, std::path::Path::new("uv"), &["run", "--no-project", "--quiet", "--with", "pandas", "python", "rt.py"]));
    assert!(o.ok() && o.stdout.contains("OK"), "siblings --json did not load into a DataFrame:\njson:\n{js}\n{o}");
}
...
fn offtree_md_edit_is_not_a_code_delta_and_does_not_bypass_duplicate_check() {
    ...
    assert!(!d.contains("REPORT.md") && !d.contains("idea.txt"), "off-tree file shows up in default diff:\n{d}");
    let s = r.ok(&["siblings", &a]).stdout;
    assert!(!s.contains("REPORT") && !s.lines().any(|l| l.trim_start_matches(['│', '|', ' ']).starts_with("code")), "off-tree edit produced a code row:\n{s}");
    let dd = r.ok(&["diff", &a, &b, "--docs"]).stdout;
    assert!(dd.contains("REPORT.md"), "`diff --docs` does not show the off-tree change:\n{dd}");
}
...
fn show_displays_offtree_snapshot_at_run_time() {
    ...
    assert!(s.contains("REPORT.md"), "show does not list the docs snapshot (§3 off-tree):\n{s}");
}
...
fn output_dirs_are_not_part_of_the_recipe() {
    let r = Repo::init();
    r.run(&["-m", "a"], "mkdir -p outputs; echo result > outputs/result.txt");
    assert!(r.root.join("outputs/result.txt").exists());
    let o = r.run_out(&["-m", "b"], "true");
    assert!(!o.ok(), "files in outputs/ changed the recipe (should be auto-ignored):\n{o}");
}
...
fn custom_output_dir_from_config_toml() {
    ...
    let o = r.run_out(&["-m", "b"], "true");
    assert!(!o.ok(), "results/ listed in output_dirs still changed the recipe:\n{o}");
}
...
fn file_over_10mb_is_excluded_from_code_with_warning() {
    ...
    assert!(o.ok(), "{o}");
    assert!(o.stderr.contains("big.bin") || o.stdout.contains("big.bin"), "no warning naming the >10 MB file:\n{o}");
}
...
fn warns_on_more_than_500_new_files() {
    let r = Repo::init();
    r.run(&["-m", "a"], "true");
    for i in 0..501 { r.write(&format!("gen/f{i}.txt"), format!("{i}")); }
    let o = r.run_out(&["-m", "b"], "true");
    assert!(o.ok(), "{o}");
    assert!(o.stderr.contains("500") || o.stderr.contains("501") || o.stderr.to_lowercase().contains("new files"), "no >500-new-files warning:\n{o}");
}
...
fn diff_shows_config_and_code_deltas() {
    ...
    assert!(d.contains("depth") && d.contains("12") && d.contains("24"), "config delta missing:\n{d}");
...
fn diff_between_siblings_uses_parent_relative_view() {
    ...
    assert!(d.contains("depth") && d.contains("act"), "sibling diff should show both parent-relative deltas:\n{d}");
}

#[test]
fn config_delta_uses_dotted_paths() {
    let r = Repo::init();
    r.write("config.yaml", "model:\n  depth: 12\n  width: 64\nopt:\n  lr: 3\n");
    let p = r.run(&["-m", "p"], "true");
    r.write("config.yaml", "model:\n  depth: 24\n  width: 64\nopt:\n  lr: 3\n");
    let c = r.run(&["-m", "c"], "true");
    let d = r.ok(&["diff", &p, &c]).stdout;
    assert!(d.contains("model.depth"), "dotted path `model.depth` missing:\n{d}");
...
fn fork_restores_code_and_config_leaves_offtree_and_prints_id() {
    ...
    assert!(!r.root.join("attn.py").exists(), "attn.py (not in parent) left behind");
    assert_eq!(r.read("NOTES.md"), "edited after the fact\n", "fork touched an off-tree file");
...
fn readme_is_offtree_by_default_and_negation_opts_in() {
    ...
    assert!(!r.run_out(&["-m", "b"], "true").ok(), "README.md edit bypassed the duplicate check");
    r.set_config("offtree", "[\"*.md\", \"notes/\", \"!README.md\"]");
    r.write("README.md", "v3\n");
    let o = r.run_out(&["-m", "c"], "true");
    assert!(o.ok(), "with `!README.md`, README.md should be code and change the recipe:\n{o}");
}
