...
- **Backup:** before migrating, `VACUUM INTO '.pollard/backup/db-format1.sqlite'`, and the notice names that path. The file is a few MB, because metrics and nodes are small. The backup must be a **standalone** copy: the repo runs in WAL mode, so moving `db.sqlite` without its `-wal` could miss recent writes. Never overwrite an existing backup (use `db-format1-<unix seconds>.sqlite`).
- **Op log barrier:** `meta.migrated_at_op = max(op_id)`. `ops::undo` refuses any request that would reach an op at or below it, whole (no partial undo; wording in BACKLOG F5). This satisfies PLAN N1: old op snapshots are never decoded into the new `Node` struct, so there is no legacy handling in `ops::decode`.
- **Crash safety:** the migration is a single transaction, so a crash leaves the repo either v1 and intact or v2 and complete.
- **Make alpha.1 binaries fail loudly (PLAN D2). Feasible, cheap:** alpha.1 opens `.pollard/db.sqlite` by fixed path. Format 2 moves the database to `.pollard/state.sqlite`, and in place of the old `db.sqlite` leaves a **directory** named `db.sqlite` containing `UPGRADED.txt`. alpha.1's `Connection::open` then fails on every command ("database: unable to open database file") before writing anything. Order: `VACUUM INTO backup/db-format1.sqlite` (standalone backup) → `VACUUM INTO state.sqlite.tmp` → migrate the tmp file → rename it to `state.sqlite` → delete the old `db.sqlite` and its `-wal`/`-shm` → `mkdir db.sqlite`. On open, a `state.sqlite.tmp` left behind is deleted and the migration is redone. Rollback: `rm -r db.sqlite state.sqlite* && mv backup/db-format1.sqlite db.sqlite`, then reinstall alpha.1. Cost: S on top of the framework. Future formats (3+) need no rename, because `> LATEST` already refuses.
...
- **Pins** become records in the segment. **Pending owner D19 (default: lossless):** one record per pin change, `{"pin": [node_id, name|null], "removed": bool}`, replayed in segment order, so concurrent pin edits from two clones both survive; only a change to the *same* pin is last-writer-wins. (Alternative if the owner declines: one `{"pins": [[node_id, name|null], …]}` line per segment, last-writer-wins on the whole map, D-29 unchanged.) The legacy `pins.json` is read only when no segment carries pins. Cost of the default: S inside R1.
- **Conversion** happens on the first push by a format-2 client: write `FORMAT`, then append one **poison line** `{"pollard_format":2,"upgrade":"…"}` to the legacy `nodes.jsonl`. alpha.1's `sync::remote_nodes` fails to parse it (`missing field node`) and errors **before** writing, on both push and pull. That makes alpha.1 fail loudly (PLAN D2). Format-2 readers skip the line.
...
| | |
|---|---|
| Approach | Section 1a/1b. Files: `pollard-core/src/repo.rs` (`open_at`, new `migrate`, `open_read_only`, db path const), `node.rs` (`pruned_at` in `Node`, `COLS`, `from_row`, `insert`; `Status::parse` returns `Result` and never silently maps to `Pruned`; the `Pruned` variant is removed), `ops.rs` (barrier in `undo`), `weights.rs`/`sync.rs`/`siblings.rs`/`tree.rs`/`run.rs` (every `Status::Pruned` / `'pruned'` use, 9 sites, switched to `pruned_at`). `tree.rs` also gets the placeholder guard from N2 in this phase (a pruned node with non-pruned descendants renders as a placeholder and recursion continues), so the migrated #19 repro is visible at the end of phase 1 (PLAN N1, BACKLOG F6); N2 only adds node-only prune on top. In phase 1 `prune` keeps its alpha.1 behaviour (whole subtree, no protection, no prompt) but writes `pruned_at` instead of the status (BACKLOG C8). |
...
| Extras | **`prune --failed` / `--killed [<node>]` (in scope, S, end of N2; owner D18):** in `weights::prune`, a selection mode instead of a target node: candidates = nodes in scope (repo, or `<node>`'s subtree) with the chosen status(es), `pruned_at IS NULL`, and no child with `pruned_at IS NULL`; computed once (one pass). Pins and `@` are dropped from the set with a notice instead of refusing. The rest goes through the same preview (grouped by `sweep`)/confirm/`-y`/`--dry-run` path and one op. `gc --older-than 30d`: deferred (S once `pruned_at` exists). |
...
| **Resumed label** (derived; owner D17) | Render-time only, no schema: for a `failed`/`killed` node, collect `fork_step` of children with `fork_step IS NOT NULL AND pruned_at IS NULL`; if any, append ` · resumed @s1 @s2…` (ascending) to the status text in `tree.rs`, `siblings.rs::render` and `pollard.rs::show`. Never for `done`/`running`, never in `--json` or the `status` column. Width fallback `resumed ×K` and colour come with #18 (N5). | S | #13 (phase 2); #18 for colour/width | None. | `n3_tree_run.rs`: Journey C gives `killed · resumed @30000` in `tree`/`show`/`siblings`; two resumes list both steps; `--parent` child without `--step` doesn't count; pruning the resume child removes the label; done node never labelled; no `resumed` in `--json`. `n5_terminal.rs`: `resumed ×K` at narrow width. |
...
| **Hydra stale config** (bug; lands in phase 2 with N3, PLAN §3) | `run.rs::capture_hydra_config`: accept only `.hydra/config.yaml` with `mtime >= run start` (pass the start time from `execute`); otherwise warn "no Hydra config written by this run" and keep the empty config. | S | none | None. | `n6_data.rs`: a stale `.hydra/config.yaml` from an earlier run is not captured. |
...
| **§12 tag/stat modes** (deferred by owner, D15) | `config.rs`: `data: Vec<DataRoot>`, `#[serde(untagged)] enum DataRoot { Path(String), Tag{tag}, Stat{path, mode} }`, so the plain string stays valid. `wc.rs::data_manifest`: Tag → one entry `path: "tag:<tag>"`, `hash = b3(tag)`, reads nothing; Stat → per-file `hash = b3(size ‖ mtime_ns)`, never reading content. `delta.rs::summarize_changes` renders a `tag:` pair as `data v2 → v3`. | M | Moved to the release after this one to keep the budget. Risk: No node schema change (§12). An unchanged plain-string config must hash byte-identically to alpha.1. Tests when it returns: `n6_data.rs`: tag mode reads nothing (a `chmod 000` dir under the root); a tag bump changes the hash and `siblings` shows `data v2 → v3`; stat mode detects add/remove/resize and misses a same-size edit with mtime restored (the limit is pinned by the test); the alpha.1 fixture's local root keeps the same hash. |
...
#13 ──> resumed label (text; colour with #18)
N2 #19 ──> prune --failed/--killed
#13, #11, #12, hydra fix, platforms: independent  (§12 deferred)
```
...
| **2. Behaviour on the new schema** | N2 #19, then `prune --failed`/`--killed`; #13; resumed label; #11; #12; #17 cmd + `--pinned`; hydra fix; R2 MinIO CI | L + S + S + S + S + S + M + S + M ≈ 9–10 days | #19 and #17 share the tree row-state model; the S-sized fixes fill in around them. |
| **3. Terminal UX** | metric_goal; #18 (incl. resumed-label colour and `×K` width fallback); #17 highlight rendering; #14 | S + L + (S) + M ≈ 5–6 days | One pass through every renderer; colour, the `◆` marker and width math designed once. |
| **4. Reach (cuttable, in this order)** | Platforms; S3 data listing | M + S ≈ 2–3 days | Independent of the rest; platforms are the first cut if the release slips, S3 data listing the second. §12 tag/stat is deferred (D15). |

**Totals.** In scope per PLAN: 3 L (N1, #19, #18) + 5 M (R1, R2, #17, #14, platforms) + 9 S (#13, resumed label, #11, #12, `prune --failed`/`--killed`, metric_goal, S3 data listing, hydra fix, draft asset) ≈ **23 engineer-days** (L = 3.5, M = 1.5, S = 0.5; 22.5 exactly), about 4½ weeks for one implementer. Deferred: #16 L, §12 M, gc objects M, #15 S, `gc --older-than` S ≈ 8–9 more days.

---

## 5. Open technical questions (all DECIDED 2026-09-19: recommended defaults accepted; see PLAN §6)
